...
- (void)layoutSubviews {
    ...
    //自定义高度时，使用
    if (customBoxHeight != 0) {
        size = CGSizeMake(size.width, customBoxHeight);
        customBoxHeight = 0;
    }
}
...
#pragma mark - JX Extension
//显示文字时，调用修改显示框的高度
- (void)setBoxHeight:(CGFloat)height {
    customBoxHeight = height;
}
...
- (void)drawRect:(CGRect)rect {
    NSLog(@"drawRect W H %f %f",rect.size.width,rect.size.height);
	CGRect allRect = self.bounds;
